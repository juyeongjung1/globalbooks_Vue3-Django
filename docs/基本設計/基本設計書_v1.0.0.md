...
### 4.4 画面遷移図
システム全体の画面遷移は以下の通りである。

```mermaid
graph TD
    Root["/ (Root)"] -->|Redirect| List["書籍一覧 (/books)"]
    List -->|「新規登録」クリック| New["新規書籍登録 (/books/new)"]
    List -->|書籍クリック| Detail["書籍詳細 (/books/:id)"]
    
    Detail -->|「編集」クリック| Edit["書籍編集 (/books/:id/edit)"]
    Detail -->|「削除」クリック| List
    Detail -->|「戻る」クリック| List
...
### 5.1 エラーハンドリング方針
- **APIエラー**: Backendからのレスポンス(4xx, 5xx)を受け取った場合、Frontend側でキャッチし、`alert()` 等を用いてユーザーに通知する。
- **バリデーション**: Frontend側での必須チェックに加え、Backend側(Serializer)でも検証を行う。不正なデータは保存されない。

### 5.2 セキュリティ方針
- **CORS**: 開発環境においては `localhost:5173` (Vite) からのアクセスのみを許可する設定 (`CORS_ALLOWED_ORIGINS`) とする。
- **CSRF**: 現状の実装では無効化するが、本番運用の際は適切なCSRF対策を行う必要がある。

### 5.3 ディレクトリ構成 (Frontend)
```
frontend/src/
  ├── components/
  │   ├── HeaderMenu.vue
  ...
  ├── views/
  │   ├── BookListView.vue     # 一覧
  │   ├── BookDetailView.vue   # 詳細
  │   ├── BookEditView.vue     # 編集
  │   └── NewBookView.vue      # 新規登録
  ├── router/
  │   └── index.ts             # ルーティング定義
  └── App.vue                  # ルートコンポーネント
```
